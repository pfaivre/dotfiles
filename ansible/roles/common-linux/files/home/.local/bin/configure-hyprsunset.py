...
def generate_sunlight_timeline(info: SunlightInfo) -> str:
    ...
    astronomical_invalid = (
        info.astronomical_twilight_begin.year == 1970
        or info.astronomical_twilight_end.year == 1970
    )

    # Check if the timeline wraps around midnight
    wraps_around = (
        astronomical_end < astronomical_begin if not astronomical_invalid else False
    )
    ...
    for i in range(total_blocks):
        ...
        if astronomical_invalid:
            ...
            if (
                current_minute >= astronomical_begin
                or current_minute < astronomical_end
            ):
...
def fetch_sunlight_info(
    latitude: float, longitude: float, day: date or None = None
) -> SunlightInfo:
    try:
        ...
        info = SunlightInfo(
            ...
            civil_twilight_begin=datetime.fromisoformat(
                response["results"]["civil_twilight_begin"]
            ),
            civil_twilight_end=datetime.fromisoformat(
                response["results"]["civil_twilight_end"]
            ),
            nautical_twilight_begin=datetime.fromisoformat(
                response["results"]["nautical_twilight_begin"]
            ),
            nautical_twilight_end=datetime.fromisoformat(
                response["results"]["nautical_twilight_end"]
            ),
            astronomical_twilight_begin=datetime.fromisoformat(
                response["results"]["astronomical_twilight_begin"]
            ),
            astronomical_twilight_end=datetime.fromisoformat(
                response["results"]["astronomical_twilight_end"]
            ),
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Configure Hyprsunset based on sunlight information."
    )
    parser.add_argument(
        "--latitude",
        "--lat",
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force writting the configuration file"
    )
