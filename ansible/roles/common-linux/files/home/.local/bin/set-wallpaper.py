...
def get_wallpaper_files() -> list[Path]:
    ...
    if not valid_wallpapers:
        print(
            f"Error: No valid wallpaper files found in {WALLPAPER_DIR}", file=sys.stderr
        )
        sys.exit(1)
...
def refresh_hyprpaper():
    ...
    try:
        ...
        result = subprocess.run(
            ["pidof", "hyprpaper"], timeout=1, capture_output=True, text=True
        )
        if result.returncode == 0:
            old_hyprpaper_pid = (
                result.stdout.strip()
            )  # Can contain multiple PIDs "8965 4325 3245"
        else:
            old_hyprpaper_pid = ""
        ...
        print(
            "Error: The hyprctl hyprpaper command timed out after 3 seconds.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def set_wallpaper(target):
    """Create a symbolic link from WALLPAPER_LINK to target, then refresh Hyprpaper to show it"""
    try:
...
def main():
    ...
    # Random mode: select a random wallpaper
        ...
        print(
            "  set-wallpaper.py          # Create link to first wallpaper (if doesn't exist)",
            file=sys.stderr,
        )
